...
function generateMerchantOrderId(userId, courseId) {
  ...
  return `ORD_${courseId}_${userId}_${rand}`; // e.g., ORD_course_user_ab12ef34
}
...
export const initiatePhonePePayment = async (req, res) => {
  try {
    ...
    const redirectBase = process.env.SITE_URL || 'http://localhost:5173';
    const redirectUrl = `${redirectBase}/payment-success?order_id=${encodeURIComponent(merchantOrderId)}`;
    const callbackUrl = `${redirectBase}/api/payments/phonepe/callback`; // If we later expose public callback
...
export const phonePeOrderStatus = async (req, res) => {
  try {
    ...
    const user = req.user;
    const required = ['PHONEPE_MERCHANT_ID', 'PHONEPE_CLIENT_SECRET'];
    const missing = required.filter(k => !process.env[k]);
    if (missing.length) {
      return res.status(500).json({ message: `PhonePe configuration missing: ${missing.join(', ')}` });
    }
    ...
    // Generate X-VERIFY for status check: SHA256(/pg/v1/status/{merchantId}/{orderId} + salt_key)
    ...
    let data;
    try {
      const resp = await axios.get(`${base}${statusEndpoint}`, { headers, timeout: 8000 });
      data = resp.data;
    } catch (apiErr) {
    ...
    const status = data?.code === 'PAYMENT_SUCCESS' ? 'SUCCESS' : data?.code;
    // On success: mark manual_payments row approved and create enrollment if not present
    if (status === 'SUCCESS' || data?.code === 'PAYMENT_SUCCESS') {
      // Fetch payment row
      ...
      if (paymentRow && paymentRow.status !== 'approved') {
        ...
        if (!existingEnroll) {
          const { data: enrollData, error: enrollErr } = await supabaseAdmin
            ...
            .select()
            .single();
          if (enrollErr) console.error('Enrollment create error (PhonePe):', enrollErr);
          else console.log('Enrollment created for PhonePe payment', enrollData?.id);
        }
        const { error: updateErr } = await supabaseAdmin
          .from('manual_payments')
          .update({ status: 'approved', processed_at: new Date().toISOString() })
          .eq('id', paymentRow.id);
        if (updateErr) console.error('Failed to update manual_payments to approved:', updateErr);
      }
    }

    res.json({ orderId, raw: data });
  } catch (err) {
