...
// Auto-enroll user in free course (price 0)
const enrollFreeCourse = async (req, res) => {
  try {
    ...
    // Fetch course price from backend DB
    const { data: course, error: courseError } = await supabaseAdmin
      .from('courses')
      .select('id, price')
      .eq('id', courseId)
    ...
    // Only allow if price is exactly 0 (number)
    ...
    res.status(200).json({ message: "Enrolled in free course", enrollment: enroll });
  } catch (err) {
    res.status(500).json({ message: "Failed to enroll in free course", error: err.message });
  }
};
...
export { 
  ...
  forgotPassword,
  getCourseById,
  enrollFreeCourse // <-- export the new function
};