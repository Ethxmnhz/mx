...
export const initiatePhonePePayment = async (req, res) => {
  try {
    ...
    const payload = {
      ...
      amount: amountPaise,
      // currency defaults to INR for PhonePe, include explicitly if required
      currency: 'INR',
      redirectUrl,
      callbackUrl,
      // store course & coupon metadata in custom fields if supported
      customMeta: {
        courseId: course.id,
        title: course.title,
        coupon: code || null
    ...
    // For the newer Standard Checkout flow, PhonePe may require an Authorization bearer token; placeholder:
    ...
    const payUrl = responseData?.data?.payPageUrl || responseData?.payPageUrl || null;
    if (!payUrl) {
