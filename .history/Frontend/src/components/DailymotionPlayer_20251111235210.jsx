...
export default function DailymotionPlayer({ embedUrl, title = 'Video', onComplete, onNext }) {
  ...
  // Simple fallback: Show overlay after estimated video duration
  // Since Dailymotion API isn't working, we'll use a manual trigger after a set time
  useEffect(() => {
    console.log('Setting up fallback timer for overlay...');
    ...
    // Show overlay after 30 seconds for testing (adjust as needed)
    // In production, you might want to make this configurable per video
    const testDuration = 30; // seconds
    const timer = setTimeout(() => {
      console.log('Fallback timer: Showing overlay after', testDuration, 'seconds');
      setShowOverlay(true);
    }, testDuration * 1000);
  ...
  // Try to listen for Dailymotion events (may not work due to API limitations)
  useEffect(() => {
    const handleMessage = (event) => {
      ...
      try {
        ...
        console.log('Dailymotion event received:', data);
        
        if (data.event === 'durationchange' && data.duration) {
          setVideoDuration(data.duration);
          console.log('Video duration:', data.duration);
          
          // Clear fallback timer if we got real duration
          if (timerRef.current) {
            clearTimeout(timerRef.current);
          }
          
          // Set new timer for 10 seconds before end
          const triggerTime = Math.max(0, (data.duration - 10)) * 1000;
          timerRef.current = setTimeout(() => {
            console.log('Auto-showing overlay 10s before end');
            setShowOverlay(true);
          }, triggerTime);
