...
export default function DailymotionPlayer({ embedUrl, title = 'Video', onComplete, onNext }) {
  ...
  // Test: Show overlay after 5 seconds for debugging
  useEffect(() => {
    console.log('DailymotionPlayer mounted, src:', src);
    const testTimer = setTimeout(() => {
      console.log('TEST: Showing overlay after 5 seconds');
      setShowOverlay(true);
    }, 5000);
    ...
    const handleMessage = (event) => {
      ...
      try {
        ...
        if (data.event === 'durationchange') {
          ...
          if (iframeRef.current?.contentWindow) {
            iframeRef.current.contentWindow.postMessage(
              JSON.stringify({ command: 'addEventListener', event: 'durationchange' }),
              'https://www.dailymotion.com'
            );
            iframeRef.current.contentWindow.postMessage(
              JSON.stringify({ command: 'addEventListener', event: 'timeupdate' }),
              'https://www.dailymotion.com'
            );
            iframeRef.current.contentWindow.postMessage(
              JSON.stringify({ command: 'addEventListener', event: 'ended' }),
              'https://www.dailymotion.com'
            );
            iframeRef.current.contentWindow.postMessage(
              JSON.stringify({ command: 'addEventListener', event: 'play' }),
              'https://www.dailymotion.com'
            );
            iframeRef.current.contentWindow.postMessage(
              JSON.stringify({ command: 'addEventListener', event: 'pause' }),
              'https://www.dailymotion.com'
            );
          }
        }
