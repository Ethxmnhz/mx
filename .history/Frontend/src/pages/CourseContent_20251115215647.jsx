...
function VideoBoostButton({ isDailymotion, videoRef }) {
  ...
  const handleBoost = async () => {
    if (isDailymotion) {
      toast.error('Cannot boost embedded video beyond its max volume');
      return;
    }

    const videoEl = videoRef?.current;
    if (!videoEl) {
      console.error('No video ref available for boosting');
    ...
    try {
      ...
      // Create media element source once
      if (!graph.source) {
        try {
      ...
      // Apply boost (cap at 3x to avoid distortion)
      graph.gain.gain.value = 3.0;
      setBoostActive(true);
      toast.success('🔊 Volume boosted (3x)');
    } catch (err) {
  ...
  return (
    <div className="flex items-center gap-3 px-3 py-2.5 bg-white/5 border border-white/10 rounded-lg">
      <button
        ...
        title={boostActive ? 'Click to turn off volume boost' : 'Click to boost volume'}
      >
                ...
                {activeContent.file_type === 'video' ? (
                  <div className="space-y-3">
                    ...
                    {(() => {
                      ...
                      // Render boost + player together so ref is stable per render
                      if (activeContent?.embed_url) {
                        return (
                          <>
                            <VideoBoostButton isDailymotion={true} videoRef={null} />
                            <DailymotionPlayer
                      ...
                      return (
                        <>
                          <VideoBoostButton isDailymotion={false} videoRef={html5Ref} />
                          <Html5VideoPlayer
                            ref={html5Ref}
