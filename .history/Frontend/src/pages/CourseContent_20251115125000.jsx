...
const CourseContent = () => {
  ...
  // Auto-scroll the right pane to show the content header/player on lesson change
  useEffect(() => {
    const anchor = contentTopRef.current;
    const container = scrollContainerRef.current;
    if (!activeContent || !anchor || !container) return;
    try {
      const anchorRect = anchor.getBoundingClientRect();
      const containerRect = container.getBoundingClientRect();
      const offset = 80; // account for top bar height
      const targetTop = anchorRect.top - containerRect.top + container.scrollTop - offset;
