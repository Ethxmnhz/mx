                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex items-center gap-3 px-3 py-2.5 bg-white/5 border border-white/10 rounded-lg">
                      <button
                        onClick={() => {
                          try {
                            if (activeContent?.embed_url) {
                              // For Dailymotion: max volume button
                              const iframes = document.querySelectorAll('iframe[title]');
                              const playerFrame = Array.from(iframes).find(f => f.title === activeContent.lesson_title);
                              if (playerFrame?.contentWindow) {
                                playerFrame.contentWindow.postMessage(JSON.stringify({ command: 'unmute' }), 'https://www.dailymotion.com');
                                playerFrame.contentWindow.postMessage(JSON.stringify({ command: 'volume', value: 1 }), 'https://www.dailymotion.com');
                              }
                            }
                          } catch (e) {
                        ...
                        className="px-3 py-1.5 rounded-md text-xs font-semibold border bg-white/10 text-slate-200 border-white/20 hover:bg-white/20 inline-flex items-center gap-2"
                        title="Boost volume to maximum"
                      >
                        🔊 Boost Volume
                      </button>
                    </div>
