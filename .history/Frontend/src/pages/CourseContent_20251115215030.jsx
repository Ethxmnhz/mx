...
function VideoBoostButton({ videoTitle, isDailymotion }) {
  ...
  const handleBoost = async () => {
    try {
      ...
      // Find the video element (always the last video on page)
      const videoEls = document.querySelectorAll('video');
      const videoEl = videoEls[videoEls.length - 1];
      ...
      if (!state.ctx) {
        ...
        console.log('✓ Audio context created:', state.ctx);
      }
      ...
      if (!state.source) {
        try {
          ...
          console.error('✗ Cannot create media source:', e.message);
          if (e.message.includes('already been associated')) {
            toast.error('Video already connected to audio context');
          } else {
            toast.error('Cannot boost this video format');
          }
          return;
        }
      }
      ...
      if (!state.gain) {
        ...
        console.log('✓ Gain node connected to audio graph');
      }

      // Set boost level to 3x
      state.gain.gain.value = 3.0;
      console.log('✓ Volume boost set to 3x');

      setBoostActive(true);
      toast.success('🔊 Volume boosted to 3x!');
    } catch (err) {
      console.error('✗ Boost error:', err);
      toast.error('Boost failed: ' + (err.message || 'Unknown error'));
    }
  };
