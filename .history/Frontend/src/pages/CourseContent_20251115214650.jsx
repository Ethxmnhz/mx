  ...
  const handleBoost = async () => {
    try {
      ...
      if (isDailymotion) {
        ...
        const videoEl = Array.from(videoElements).find(v => v.title === videoTitle);
        ...
        if (!gainRef.current) {
          ...
          gainRef.current.gain.value = 2.0; // 2x volume boost
      ...
      toast.error('Boost not available');
    }
  };
  ...
  return (
    <div className="flex items-center gap-3 px-3 py-2.5 bg-white/5 border border-white/10 rounded-lg">
      ...
      >
        ...
        {boostActive ? 'Boost On (2x)' : 'Boost Volume'}
      </button>
    </div>
  );
}
