...
function VideoBoostButton({ isDailymotion, videoRef }) {
  // Gain levels to cycle through; first entry is normal volume
  const gainLevels = [1.0, 1.5, 2.0, 3.0];
  const [levelIndex, setLevelIndex] = useState(0); // 0 means OFF / normal
  const audioGraphRef = useRef({ ctx: null, source: null, gain: null });

  const cycleBoost = async () => {
    if (isDailymotion) {
      toast((t) => (
        <span>Embedded player can't be boosted beyond its own volume.</span>
      ));
      return;
    }
    ...
    const nextIndex = (levelIndex + 1) % gainLevels.length;
    setLevelIndex(nextIndex);

    try {
      const graph = audioGraphRef.current;
      const desiredGain = gainLevels[nextIndex];

      // If returning to 1.0 just set and exit (reuse existing graph if any)
      if (desiredGain === 1.0 && graph.gain) {
      ...
      if (!graph.ctx) {
        graph.ctx = new (window.AudioContext || window.webkitAudioContext)();
      }
      if (graph.ctx.state === 'suspended') {
        await graph.ctx.resume();
      }

      // Media element source once
      if (!graph.source) {
        try {
          graph.source = graph.ctx.createMediaElementSource(videoEl);
        } catch (e) {
      ...
      // Apply desired gain
      graph.gain.gain.value = desiredGain;
      toast.success(desiredGain === 1.0 ? 'Boost OFF' : `🔊 Boost ${desiredGain.toFixed(1)}x`);
    } catch (err) {
  ...
  const activeGain = gainLevels[levelIndex];
  const isActive = activeGain !== 1.0;
  const label = isDailymotion
    ? 'Embed Volume'
    : isActive
      ? `🔊 ${activeGain.toFixed(1)}x`
      : '🔇 Boost';
    ...
    <div className="flex items-center gap-3 px-3 py-2.5 bg-white/5 border border-white/10 rounded-lg">
      <button
        ...
        className={`px-3 py-1.5 rounded-md text-xs font-semibold border transition-all inline-flex items-center gap-2 ${
          isDailymotion
            ? 'bg-white/10 text-slate-300 border-white/20 cursor-not-allowed'
            : isActive
        ...
        title={isDailymotion ? 'Embedded player cannot be boosted' : 'Cycle boost levels'}
        disabled={isDailymotion}
      >
...
const CourseContent = () => {
  ...
  // Stable ref for HTML5 video element (used by boost button)
              ...
              <div className="p-6">
                {activeContent.file_type === 'video' ? (
                  <div className="space-y-3">
                    <VideoBoostButton
                      isDailymotion={!!activeContent?.embed_url}
                      videoRef={activeContent?.embed_url ? null : html5VideoRef}
                    />
