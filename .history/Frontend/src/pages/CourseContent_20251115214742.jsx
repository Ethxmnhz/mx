...
function VideoBoostButton({ videoTitle, isDailymotion }) {
    ...
    try {
      if (boostActive) {
        ...
        if (isDailymotion) {
          ...
          const playerFrame = Array.from(iframes).find(f => f.title === videoTitle);
          if (playerFrame?.contentWindow) {
            playerFrame.contentWindow.postMessage(JSON.stringify({ command: 'volume', value: 0.5 }), '*');
          }
        }
      ...
      if (isDailymotion) {
        // For Dailymotion: send unmute + max volume commands
        const iframes = document.querySelectorAll('iframe[title]');
        const playerFrame = Array.from(iframes).find(f => f.title === videoTitle);
        if (playerFrame?.contentWindow) {
          playerFrame.contentWindow.postMessage(JSON.stringify({ command: 'unmute' }), '*');
          playerFrame.contentWindow.postMessage(JSON.stringify({ command: 'volume', value: 1 }), '*');
          setBoostActive(true);
          toast.success('Volume boosted to MAX!');
        } else {
          toast.error('Could not find player');
        }
      } else {
        // For HTML5 video: use Web Audio API
        const videoElements = document.querySelectorAll('video');
        const videoEl = videoElements[videoElements.length - 1]; // Get last video (most recent)
        ...
        if (!sourceRef.current) {
          try {
            sourceRef.current = ctx.createMediaElementSource(videoEl);
            console.log('Media element source created');
          } catch (e) {
            console.error('Source creation error:', e);
            toast.error('Cannot boost this video');
            return;
          }
        }
        ...
        if (!gainRef.current) {
          ...
          gainRef.current.gain.value = 2.5; // 2.5x volume boost
          sourceRef.current.connect(gainRef.current);
          gainRef.current.connect(ctx.destination);
          console.log('Gain node connected, boost = 2.5x');
        } else {
          gainRef.current.gain.value = 2.5;
        }

        setBoostActive(true);
        toast.success('Volume boosted to 2.5x!');
      }
    } catch (err) {
  ...
  return (
    <div className="flex items-center gap-3 px-3 py-2.5 bg-white/5 border border-white/10 rounded-lg">
      <button
        ...
        title={boostActive ? 'Click to turn off volume boost' : 'Click to boost volume to maximum'}
      >
        <SpeakerWaveIcon className="w-4 h-4" />
        {boostActive ? 'Boost On (2.5x)' : 'Boost Volume'}
      </button>
    </div>
  );
}
              ...
              <div className="p-6">
                {activeContent.file_type === 'video' ? (
                  <div className="space-y-3">
                    ...
                    <VideoBoostButton videoTitle={activeContent.lesson_title} isDailymotion={!!activeContent?.embed_url} />
